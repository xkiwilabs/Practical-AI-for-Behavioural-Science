...
- **Deep learning** is a subset of ML that uses structures called neural networks — layers of basic mathematical operations inspired (loosely) by how neurons in the brain process information. Deep learning is what powers most of the impressive AI you've seen in the news.

- **Generative AI (GenAI)** is the subset you've probably interacted with most — systems like ChatGPT, Claude, and Gemini that can generate new text, images, or code based on what they've learned from vast amounts of training data.
...
**Prompt engineering** is the art of writing clear, effective instructions for an AI. A vague prompt gets a vague answer. A specific, well-structured prompt gets useful output. Compare these two:
...
The second prompt tells the AI exactly what data to use, what type of plot you want, what visual features to include, and which library to use. It will almost certainly give you working code on the first try. The first prompt? You'll be going back and forth for a while.
...
- **Be specific** about what you want, including format and details
- **Give context** — describe your data, your tools, and your goal
...
**The key insight:** You are always in control. You decide what to build, you verify the output, you judge whether it's correct. The AI is a powerful tool, but it doesn't understand your research question the way you do.
...
**4. ML for understanding human decision-making.** Auletta, Kallen, di Bernardo, and Richardson (2023) used LSTM neural networks and explainable AI (SHAP) to predict and understand human action decisions during a collaborative herding task. The ML models could predict expert and novice decisions at timescales preceding conscious intent, and the explainable AI analysis revealed that experts were more attuned to their co-actor's behaviour — something traditional analysis methods would have missed. Published in *Scientific Reports*.

A word of caution: these are tools, not magic. Good research questions, careful experimental design, and critical thinking are more important than ever. ML can find patterns in noisy data — including patterns that aren't real. Throughout this course, we'll spend as much time learning to evaluate and question ML results as we will building models.
...
5. Sign up for at least one AI assistant (ChatGPT, Claude, or Gemini)
