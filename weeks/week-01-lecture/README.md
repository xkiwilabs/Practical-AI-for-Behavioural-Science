...
2. **PLAN** — Review the AI's plan. Does the approach make sense? Is it using the right methods, the right libraries, the right steps? If not, redirect *now* — before any code is written or output is generated. This is where your domain knowledge matters most. A few minutes reviewing a plan can save you from going down the wrong path entirely.
3. **GENERATE** — Once you're happy with the plan, ask the AI to execute it. This might mean generating code, writing text, producing a visualisation, creating an analysis pipeline, or any other output. The key is that generation follows a reviewed plan, not a blind prompt.
