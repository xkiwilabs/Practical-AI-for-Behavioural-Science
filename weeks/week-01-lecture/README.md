# Week 1: Why ML and AI Belongs in the Cognitive, Psychological and Behavioural Sciences
...
The lecture slides for this week are available here: **<a href="https://xkiwilabs.github.io/Practical-AI-for-Behavioural-Science/weeks/week-01-lecture/slides/index.html" target="_blank">Week 1 Slides</a>**
...
**Foundation models and model sizes.** The AI tools above are powered by **Large Language Models or LLMs** — large neural networks trained on massive amounts of data that can be adapted to many different tasks. As of early 2026, the leading foundation models include OpenAI's **GPT-5** family (released August 2025, with GPT-5.2 following in December), Google's **Gemini 3** (November 2025), and Anthropic's **Claude** family (including Claude Opus 4.6, the most capable, and faster/cheaper models like Claude Sonnet 4.5 and Claude Haiku 4.5). Each company offers models in different sizes — sometimes called "flavours" — that trade off between capability and speed. Larger models reason more carefully but take longer and cost more to run; smaller models are faster and cheaper but less capable. This is a practical tradeoff you'll encounter throughout the course.
...
1. **ENGINEER** — Craft your prompt: be specific, provide context, state your goal clearly, and, crucially, ***ask for a plan***. Context might mean describing your data, but it can also mean uploading a document, pasting in an error message, or even asking the AI to do a web search to find relevant documentation first. Tell the AI what you want it to achieve and ask it to outline an approach *before* it generates anything.
2. **PLAN** — The AI responds with a proposed approach. Read it critically — does it make sense? Is it using the right methods, the right libraries, the right steps? If not, redirect *now*, before any code is written or output is generated. If the AI skipped the plan and jumped straight to code, ask it to step back and outline its approach first. This is where your domain knowledge matters most. A few minutes reviewing a plan can save you from going down the wrong path entirely.
3. **GENERATE** — Once you're happy with the plan, have the AI carry it out. This might mean writing code, producing text, creating a visualisation, or building an analysis pipeline. For complex tasks, it might mean generating a more detailed sub-plan before producing the final output. The key is that generation follows a reviewed plan, not a blind prompt.
4. **VERIFY** — Read what the AI gives you. Don't just copy and paste — look at what it's doing. Then run it. Does it execute without errors? Does the output make sense? Check it against what you know about your data and your research question.
5. **REFINE** — If it's not right: What went wrong? Add more context, correct misunderstandings, try a different approach. Go back to the appropriate step — sometimes you need to refine the plan, sometimes just the generated output.

**Planning is iterative.** Don't expect to nail the plan on the first try. Your first trip around the inner loop might be entirely about the plan itself — you ENGINEER a prompt asking for an approach, the AI GENERATES a proposed plan, you VERIFY it against what you know about your data and research question, and you REFINE by asking for adjustments. You might go around two or three times before the plan feels solid. Only then do you shift gears — asking the AI to GENERATE the actual code, text, or analysis. A typical task might involve 2–3 planning iterations followed by 1–2 execution iterations. That's not inefficiency; that's the process working as intended.

Even experienced developers and data analysts iterate with AI tools. But "iterate" doesn't mean blindly re-prompting and hoping for the best. Here are some strategies that work well:
...
- **Refine at the right level.** Sometimes the output is wrong because the *plan* was wrong — go back to PLAN. Sometimes the plan was fine but the AI made a coding mistake — go back to GENERATE with a correction. Knowing which level to fix saves time.
...
> **Think about it:** What does it mean that an ML model can predict a human's action decision *before the human consciously makes it* — for example, predicting which way a player will move in a sports game hundreds of milliseconds before they act? At first glance, this seems to undermine free will. But consider another interpretation: human behaviour, even when it feels spontaneous and unpredictable, may be far more structured and deterministic than we realise. Complex systems — from weather to flocking birds to human coordination — can appear noisy and irregular on the surface while being governed by deterministic dynamics underneath. This idea has deep roots and a long history in the behavioural sciences — from the self-organisation of brain and behaviour ([Kelso, 1995](https://mitpress.mit.edu/9780262611312/dynamic-patterns/)), to the dynamics of development ([Thelen & Smith, 1994](https://doi.org/10.7551/mitpress/2524.001.0001)), to fractal structure in cognition ([Van Orden, Holden, & Turvey, 2003](https://doi.org/10.1037/0096-3445.132.3.331)), to the coupling of perception and action with the environment ([Warren, 2006](https://doi.org/10.1037/0033-295X.113.2.358)), to social coordination and interpersonal dynamics ([Richardson, Dale, & Marsh, 2014](https://doi.org/10.1017/CBO9780511996481.015)). The apparent randomness in all these domains isn't randomness at all — it's complexity. If ML can learn to read those patterns, what does that tell us about the nature of human behaviour itself?
